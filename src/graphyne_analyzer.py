...
to identify a connected gamma graphyne network. It removes dangling and isolated
atoms, detects six-membered carbon rings (benzene rings), identifies triple bonds
based on distance and geometry, and classifies the remaining bonds as single bonds.
...
1. Standalone (CLI or interactive mode)
2. As a callable module inside a larger workflow/pipeline
...
def main(input_file=None, n_cpu=1, output_dir=None):
    ...
    print(f"Total atoms removed: {total_removed} "
          f"({removed_dangling} terminal or isolated carbon atoms)")
    ...
    print(f"\nOutput file successfully written to: {output_file}")
    return output_file
