...
BOND_MAX = 1.60 	# Maximum distance to consider two carbon atoms as bonded
...
def write_all_triangle_clusters(triangles, clusters, rings, atoms, coords,
    ...
    total = len(clusters)
    print(f"\nTotal number of triangle clusters to be generated: {total}")
    print(f"Using {n_cpu} CPU cores for clusters generation\n")
    ...
    print(f"\nAll triangle clusters written successfully to: {outdir_full}")
...
def generate_all_triangle_clusters(input_filename,
    ...
    print(f"Triangle cluster generation completed.")
